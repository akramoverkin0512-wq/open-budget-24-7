...
def get_db_connection():
    try:
        ...
        logging.error(f"MySQL ulanishida xato: {e}")
        return None
...
async def help_cmd(message: types.Message):
    await message.answer("❓ **Yordam bo'limi**\n\nAdmin: @Erkin_Akramov", parse_mode="Markdown")

@dp.message(F.text == "🏆 Reyting")
async def show_rating(message: types.Message):
    conn = get_db_connection()
    if not conn: return await message.answer("Baza bilan aloqa yo'q.")
    cursor = conn.cursor(dictionary=True)
...
async def reject(callback: types.CallbackQuery):
    if callback.from_user.id == ADMIN_ID:
        uid = int(callback.data.split("_")[1])
        await bot.send_message(uid, "❌ Skrinshotingiz rad etildi.")
